...
function GuildWarTimeCheck(tm)
    ...
    if tostring(tm) == "guildb" then
        ...
        if (DayOfWeek == 2 and Hour == 13) or (DayOfWeek == 4 and Hour == 7) then 
            return 1;
        else
            return 0;
        end
    elseif tostring(tm)== "guildcandeclare" then
       ...
       if (DayOfWeek == 2  and Hour >= 12 and Hour <= 13) or (DayOfWeek == 4 and Hour >= 6 and Hour <= 7)  then
           return 0;
       else
           return 1;
       end
    elseif tostring(tm)== "guildbafter" then
      --在帮会战时间内 以及当天 帮战时间后
        if (Hour >= 13 and DayOfWeek == 2) or (Hour >= 7 and DayOfWeek == 4) then
            return 1;
        else
            return 0;
        end
...
function GetNextGuildWarTime()
    ...
    local nexttmstr="";
    if tag==0 then
        nexttmstr="20:00-21:00";
    elseif tag ==2 then
        nexttmstr="13:00-14:00";
    elseif tag ==4 then
        nexttmstr="7:00-8:00";
    end


    local strNextData=string.format("%s-%s-%s %s",tostring(Year),tostring(Month),tostring(Day),tostring(nexttmstr));
