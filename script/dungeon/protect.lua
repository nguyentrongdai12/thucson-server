        ...
        if aliveMassesCount == 0 then
            if (self.CurrMassLevel >= TD_MaxSpawnLevel) then
				...
				if (DungeonIsTrainMode(dungeon) == 1) then --当前为训练模式
					...
					local standExp = self.TD_Exp * 0.5 ;
					local standNimbus = self.TD_Nimbus * 0.5*3 ;
    ...
    if (DungeonIsTrainMode(dungeon) ~= 1) then
        ...
        if (winner == DUNGEON_FACTION1) then -- 赢了	
            ...
            for i=0,total-1 do
                ...
                AddExp (actor,self.TD_Exp*0.5,0);
                AddNimbus (actor,self.TD_Nimbus*0.5*3,0);
            ...
            contentws = string.gsub(contentws, "$TD_Exp", math.floor(self.TD_Exp*0.5));
            contentws = string.gsub(contentws, "$TD_Nimbus", math.floor(self.TD_Nimbus*0.5*3));
        ...
        elseif (winner == DUNGEON_FACTION2) then -- 输了
            ...
            if (self.CurrMassLevel >= 1) then
                ...
                contentls = string.gsub(contentls, "$TD_Exp", math.floor(self.TD_Exp*0.5));
                contentls = string.gsub(contentls, "$TD_Nimbus", math.floor(self.TD_Nimbus*0.5*3));
...
function CDungeon_TD:SpawnMass()
    ...
    CreateNpcInDungeon(self.ObjectId, TD_SpawnNpc[self.CurrMassLevel], DUNGEON_FACTION2, x, y, 0, self.ActorLevel , 0.4+self.CurrMassLevel * 0.2, 0.4+self.CurrMassLevel * 0.2)
