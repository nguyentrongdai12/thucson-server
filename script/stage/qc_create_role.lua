local bDebug    = false
local fForward  = 307.19805908203
local fYaw      = -2.6300549507141e-005
local fRoll     = 7.8914284706116
local fUpOffset = 90.5

function OnStart()
    --RenderAddActor("login", "login002")
    ActorAdd("login", "longin_qingcx01")
    ActorPlayPose("login", "scene", true)

    CameraSetProjParams(50, 1.333, 1.0, 6000.0);
    CameraSetPitch(0);
    CameraSetYaw(fYaw);
    CameraSetRoll(fRoll);
...
function OnBeforeRender() -- 开始渲染场景
    ...
    RenderSetFog(214, 214, 195, 150, 3500)
end
